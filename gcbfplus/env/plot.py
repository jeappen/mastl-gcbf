...
def render_video(
    ...
    if render_extra is not None:
        ...
        render_extra(ax, rollout, viz_opts)
    ...
    if viz_opts.get('plot_snapshot', False):
        ...
        ax.legend(handles=[agent_circs[0], agent_circs_faded[0]], loc='upper right', fontsize=16,
                  handler_map={plt.Circle: HandlerCircle()})

        # Show legend
        # ax.legend(loc='upper right', fontsize=16)
        # Save the snapshot with new filename
        # snapshot_path = video_path.with_name("demo.pgf")  # Save as pgf for LaTeX
        if viz_opts.get('plot_pgf', False):
