...
class GCBFPlus(GCBF):
    ...
    def __init__(
            ...
            seed: int = 0,
            online_pol_refine: bool = False,  # Only for prioritize safety mode
            **kwargs
    ):
    ...
    def act(self, graph: GraphsTuple, params: Optional[Params] = None) -> Action:
        ...
        if self.online_pol_refine:
            # Refinement is expensive but can give best action
            # action = self.online_policy_refinement(graph, params)
            action = 2 * self.actor.get_action(params, graph) + self._env.u_ref(graph)
            return self._prioritize_safety(graph, params, action)
        action = 2 * self.actor.get_action(params, graph) + self._env.u_ref(graph)
        return action

    def _prioritize_safety(self, graph: GraphsTuple, params: Optional[Params] = None, action: Action = None) -> Action:
        """If action is unsafe, refine it to be safe. For the prioritized safety mode."""
        if params is None:
            params = self.actor_train_state.params

        # One step lookahead for safety
        next_graph_action = self._env.forward_graph(graph, action)
        unsafe_mask = self._env.unsafe_mask(next_graph_action)
        return jax.lax.cond(unsafe_mask.any(), lambda s, a, u: self._env.stay_stationary(s, a, u),
                            lambda s, a, u: a, graph.type_states(type_idx=MultiAgentEnv.AGENT, n_type=self.n_agents),
                            action,
                            unsafe_mask)
